...
test_message:     db 'Room temp:', 0
value_message:    db 'Temp diff:', 0
cseg
; D4 is 11 
...
loop_sensor:
	...
	Load_y(40960)
	lcall mul32 ; x=x*y=ADC_OP07*4.096
	
	mov y+0, VAL_LM4040+0
	mov y+1, VAL_LM4040+1
	mov y+2, #0
	mov y+3, #0
	lcall div32 ; x=x/y=ADC_OP07*4.096 / ADC_ref
	
	Load_y(4100) ; y=is this correct?
	lcall div32 ; x=x/y=ADC_OP07*4.096 / (ADC_ref*41*10^(-6)*100)
	...
	lcall hex2bcd
    lcall Display_Pin20_BCD
	;
    
	; calculations for T_C (temperature at cold junction)
	; Vout=ADC*Vref / VAL_LM4040, VAL_LM4040 is ADC_ref, VAL_LM335 is ADC
    ; Convert to voltage
	; ADC_LM335
	mov x+0, VAL_LM335+0
	mov x+1, VAL_LM335+1
	...
	; Vref
	Load_y(40960) ; The MEASURED voltage reference: 4.0959V, with 4 decimal places 40959 40960
	; ADC_LM335 * Vref
	lcall mul32 ; x=x*y, x=ADC_LM335 * Vref
	
	
	; ADC_LM4040 = ADC_ref
	mov y+0, VAL_LM4040+0 ; ADC_ref
	mov y+1, VAL_LM4040+1
	; Pad other bits with zero
	mov y+2, #0
	mov y+3, #0
	; ADC_LM335 * Vref / ADC_ref
	lcall div32 ; V_LM335
	
	; T_C = 100.0(V_LM335-2.73)
	Load_y(27700) ; changed from 27300 to 27700
	lcall sub32 ; x=V_LM335-2.73
	Load_y(100)
	lcall mul32 ; x=T_C
